--SourPatch Lumber Tycoon 2 v1.0.6

--> Open Sourced by SourPatchs

--> Credit if you use any code :)

repeat wait()until game:IsLoaded()and game.Players.LocalPlayer.Character;
local lib=loadstring(game:HttpGet('https://SourPatchdevelopment.wtf/lua/SourPatchUILib.lua'))();
local requirements={
    ConfigSetup={
    AlwaysDay=false, 
    AlwaysNight=false,
...
table.insert(requirements.SignTable,'SourPatch '..requirements.CurrentVersion..' Is Now Live!');
...
local functions={};
local config
if not pcall(function() readfile('SourPatch.cfg') end) then
    writefile('SourPatch.cfg',game:GetService'HttpService':JSONEncode(requirements.ConfigSetup))--table
end

config=game:GetService'HttpService':JSONDecode(readfile('SourPatch.cfg'))

functions.saveConfig=function()
    writefile('SourPatch.cfg',game:GetService'HttpService':JSONEncode(config))
end;
...
functions.updateChangelog=function(header,text)
    ...
    updateText.Text=' SourPatch: '..text
...
functions.AntiAFK=function()
    requirements.LP.Idled:Connect(function()
        functions.notify('SourPatch', 'Anti AFK Auto-Enabled', 4)
...
functions.donate=function(plr,amount)
    ...
    game:GetService'RunService'.Heartbeat:wait();
    scr2.exitNotice();
    functions.notify('SourPatch','Attempting Donation',2);
    wait(6)
    ...
    functions.notify('SourPatch','Donated '..tostring(amount)..' to '..tostring(plr),4);
    scr2.exitNotice();
end; 
...
functions.applyLight=function(value)
    if value then 
        ...
        light.Name='SourPatch'
        light.Range = 150
        light.Brightness = 1.7
    else
        pcall(function()
            requirements.LP.Character.Head.SourPatch:remove();
        end);
    end;
end;
...
functions.OpenSelectedItem=function(item)
    ...
    if itemBox and item:FindFirstChild('Type')and item.Type.Value~='Structure' then
        ...
        functions.notify('SourPatch','Opened Item',4)
    end;
end;
...
functions.saveBaseToFile=function(name)
    ...
    if not functions.getPlotCFrame()then 
        functions.notify('SourPatch',tostring(requirements.Strings.autoBuildPlayer..' Has No Plot!'));
        return;
    end;
    for _,plot in next,workspace.Properties:children()do 
    ...
    writefile(name..'.SourPatch',blueprints)
end;
...
functions.loadFile=function(Name,value)
    local localplot;
    if not pcall(function() readfile(Name..'.SourPatch') end) then
        if value then
    ...
    local blueprints={};
    if value then 
        tableS=functions.importFiles(readfile(Name..'.SourPatch'))
...
functions.LoadSelectedSlot=function(num)
    ...
    functions.notify('SourPatch', 'Loading slot '..requirements.Strings.selectedPlotToLoad, 4);
...
functions.SaveSelectedSlot=function()
    ...
    if success then
        functions.notify('SourPatch', 'Saved Slot '..requirements.LP.CurrentSaveSlot.Value,4);
    else
...
functions.manyAxeFloor=function()
    if not requirements.wrkspc:FindFirstChild('SourPatchV3')then 
...
functions.DupeInventory=function(Quantity)
    for i=1,Quantity do 
        if not requirements.Booleans.axeDuping then 
            functions.notify('SourPatch','Dupe Inventory Aborted!',4);
            return;
        end;
...
functions.AutoDupeMoney=function(amount,slot)
    ...
    for i=1,amount do
        ...
        if Money~=0 then 
            functions.notify('SourPatch','Waiting for donation...',4);
        ...
        functions.notify('SourPatch','Loading slot...',4);
        ...
        amount=amount-1;
        if amount==0 then 
            functions.notify('SourPatch','Auto dupe money finished.',5);
...
functions.PlotLogic=function()
    ...
    if not requirements.remotes.clientMayLoad:InvokeServer(requirements.LP)then
        ...
        repeat
            wait(5)
        until requirements.remotes.clientMayLoad:InvokeServer(requirements.LP);
        functions.notify('SourPatch', 'Loading plot...', 4);
    end;
    return true;
end;
...
functions.donateBaseAutoload=function(slotnum)
    ...
    getsenv(game:GetService('Players').LocalPlayer.PlayerGui.PropertyPurchasingGUI.PropertyPurchasingClient).enterPurchaseMode=function(...)
        if not autoLoading then
            ...
            functions.notify('SourPatch','Fucking working i think idk')
            for i,v in next,game.Workspace.Properties:children()do
                if v.Owner.Value==nil and not v:FindFirstChild'Square'then
...
functions.PlankToBlueprint=function()
    ...
    local target;
    functions.notify('SourPatch','Select a plank and a blueprint to continue',2);
    requirements.connections.PlankToBlueprint=requirements.Mouse.Button1Down:Connect(function()
        ...
        if target.Parent:FindFirstChild('Type')and target.Parent.Type.Value=='Blueprint'then 
            ...
            functions.notify('SourPatch','Blueprint Selected',2);
        end;
        if tostring(target.Parent)=='Plank'and target.Parent:FindFirstChild('Owner') and tostring(target.Parent.Owner.Value)==tostring(requirements.LP)then 
            ...
            functions.notify('SourPatch','Plank Selected',2);
        end;
    end);
    ...
    functions.notify('SourPatch','Done!',2);
...
functions.spawnCar=function()
    local remote;
    functions.notify('SourPatch','Click A Vehicle Spawn Pad',4);
    connection=requirements.Mouse.Button1Down:Connect(function()
        ...
        if target and target.Parent:FindFirstChild('Type')and target.Parent.Type.Value=='Vehicle Spot'then 
            ...
            connection:Disconnect();
            connection=nil;
            functions.notify('SourPatch','Spawning...',4);
        end;
    end)
    ...
    functions.notify('SourPatch','Spawned Color')
end;
...
functions.AutoBuyItem=function(item,quantity)
    ...
    for i=1,quantity do
        if requirements.Booleans.abortAutobuy then
            ...
            requirements.Booleans.isBuying=false;
            functions.Teleport(CFrame.new(oldpos.p));
            functions.notify('SourPatch','Aborted Autobuy!',4);
            return false;
        end;
...
functions.bringTree=function(Tree,Quantity)
    ...
    if Tree=='LoneCave'then
        if functions.getBestAxe().ToolName.Value~='EndTimesAxe'then
            ...
            functions.notify('SourPatch','You need an EndTimesAxe! Aborting...',4);
            return;
        end;
    end;
    local oldPos=requirements.LP.Character.HumanoidRootPart.CFrame;
    for i=1,Quantity do
        if not requirements.Booleans.isLoopBringTreeEnabled then
            ...
            functions.Teleport(CFrame.new(oldPos.p));
            functions.notify('SourPatch','Bring Tree Aborted',4);
            return
        end;
    ...
    functions.notify('SourPatch','Done',4)
    requirements.ConfigSetup.oldNoclipEnabled=false;
    requirements.Booleans.Noclip=false;
end;
...
functions.whisperToTarget=function()
    if requirements.Booleans.alertTarget then 
        ...
        requirements.remotes.sayMessageRequest:FireServer('/w '..tostring(requirements.Strings.BaseRecipient)..' load your base over my base to claim it! [Powered By SourPatch]','All');
    end;
end;

functions.tellDiscord=function()
    requirements.remotes.sayMessageRequest:FireServer('SourPatch and the tag is 4476','All');
end;
...
functions.unloadSlot=function()
    ...
    functions.notify('SourPatch','Unloaded Selected Slot Succesfully!',4);
end;
...
functions.DupeBaseToServer=function()
    ...
    functions.notify('SourPatch','Please wait. This will take a minute or two',30)
...
functions.DonatePlotToPlayer=function()
    ...
    if success then 
        ...
        if newSuccess then 
            ...
            functions.notify('SourPatch','Automatically Whitelisted '..tostring(requirements.Strings.BaseRecipient),4);
            functions.whisperToTarget();
            functions.notify('SourPatch','Successfully donated base to '..tostring(requirements.Strings.BaseRecipient),4);
            requirements.Booleans.DonatingBase=false;
        end;
    end;
end;
...
functions.ClaimAllFreePlots=function()
    ...
    functions.notify('SourPatch','Claimed All Plots!',4);
end;

local args={...};
local success=args[1]
if not success then
    functions.notify('SourPatch','You may have paid for this script when it\'s Free! that\'s not good. Chargeback :) reloading real SourPatch...');
    functions.notify('SourPatch','Please ignore this message if this doesn\'t refer to you')
    return loadstring(game:HttpGetAsync'https://raw.githubusercontent.com/FindFirstSourPatch/SourPatchV3Public/main/LumberTycoon2')('SourPatch');
end
...
functions.TriggerAllPressurePlates=function()
    ...
    if not found then
        functions.notify('SourPatch','No Pressure Plates Located',4);
        return;
    end;
end;

functions.SourPatchDetection=function()
    if functions.isPlayerAdmin(requirements.LP)then 
        functions.notify('SourPatch','Authorised Administrator Detected');
        local SourPatchUsers={};
        spawn(function()
            while wait(2)do 
                for i,v in next,game.Players:children()do 
                    if functions.WhitelistCheck(v)then 
                        if v~=requirements.LP and not table.find(SourPatchUsers,v)then
                            functions.notify('SourPatch',tostring(v)..' Has Whitelisted You',4);
                            table.insert(SourPatchUsers,v);
                        end;
                    end;
                end;
            end;
...
functions.FarAxeEquip=function()
    ...
    if requirements.connections.farAxeEquip==nil then 
        ...
        functions.notify('SourPatch','Equipped',4);
...
functions.manageTarget=function(option,plr)
    ...
    requirements.LP.Character.Humanoid.Name='SourPatchV3'
    local HClone = requirements.LP.Character.SourPatchV3:Clone()
    ...
    requirements.LP.Character.SourPatchV3:Destroy()
    local connection;
...
functions.baseDropItem=function(item,player,amountg,customPos)
        ...
        if v:IsA('Model')then 
            ...
            if check and check.Value==item and v:FindFirstChild('Owner')and tostring(v.Owner.Value)==Owner or check2 and check2.Value==item and v:FindFirstChild('Owner')and tostring(v.Owner.Value)==Owner  then
                ...
                if success then
                    ...
                    if not workspace:FindFirstChild('SourPatchV3BasedropCord')then
                        ...
                        game.ReplicatedStorage.PlaceStructure.ClientPlacedStructure:FireServer(nil, CFrame.new(workspace:FindFirstChild('SourPatchV3BasedropCord').CFrame.p),requirements.LP, nil, v, true);
                    end;
                end;
                if not success then
                    ...
                    if not workspace:FindFirstChild('SourPatchV3BasedropCord')then
                        ...
                        v:SetPrimaryPartCFrame(workspace:FindFirstChild('SourPatchV3BasedropCord').CFrame);
                    end;
                end;
                if amount==amountg and amountg~=0 and not customPos then 
                    functions.Teleport(CFrame.new(oldPos.p))
                    functions.notify('SourPatch',tostring(amount),4);
                    return;
                end;
            end;
        end;
    ...
    if amount~=0 then 
        if workspace:FindFirstChild'SourPatchV3BasedropCord'then 
            functions.Teleport(CFrame.new(workspace:FindFirstChild'SourPatchV3BasedropCord'.CFrame.p));
...
functions.getButton=function(Text)
    for _,ScrollingFrame in next,game.CoreGui.SourPatch.MainBody:GetChildren()do 
...
functions.selfDupeCurrentSlot=function(slot)
    ...
    if success then 
        ...
        functions.notify('SourPatch','Basedropping Duped Items',4)
        for _,item in next,workspace.PlayerModels:children()do 
...
local MainPlayerOptions=MainPlayerOptionsFrame:Section('SourPatch '..requirements.CurrentVersion);
...
MainPlayerOptions:KeyBind(
    ...
    function(v)
        ...
        if requirements.Booleans.Noclip then
            requirements.Booleans.Noclip=false;
            functions.notify('SourPatch','NoClip Disabled',4);
        elseif not requirements.Booleans.Noclip then
            requirements.Booleans.Noclip=true;
            functions.notify('SourPatch','NoClip Enabled',4);
        end;
    end,
...
MainPlayerOptions:Toggle(
    ...
    function(value)
        if not value then 
            ...
            functions.notify('SourPatch','Infinite Jump Disabled',4);
        elseif value then 
            ...
            functions.notify('SourPatch','Infinite Jump Enabled',4);
        end;
    end,
    false
)
...
requirements.services.userInputService.InputEnded:Connect(function(key,processed)
    if key.KeyCode==Enum.KeyCode[requirements.ConfigSetup.SprintKey]and not processed then 
        if requirements.Strings.WalkSpeed>16 then 
            ...
            requirements.Strings.WalkSpeed=tonumber(game.CoreGui.SourPatch.MainBody.ScrollingFrame.WalkSpeedSlider.SliderDarkOutline.SliderValue.Text)
        end;
    end;
end);
...
MainAxeOptions:DropDown(
    'Count Axes',
    function(axe)
        functions.notify('SourPatch','You Have '..tostring(functions.CountItem(tostring(axe)))..' '..tostring(axe)..'s',4);
    end,
...
MainAxeOptions:Toggle(
    ...
    function(v)
        if v then 
            functions.notify('SourPatch','Hit Any Tree',4);
...
MainAxeOptions:Toggle(
    ...
    function(toggle)
        if toggle then 
            functions.notify('SourPatch','Select an item to Open',4)
        ...
        elseif not toggle then
            ...
            functions.notify('SourPatch','Open Items Disabled',4)
...
BaseDropOptions:Button(
    ...
    function()
        pcall(function()
            workspace.SourPatchV3BasedropCord:Destroy();
        end)
        local SourPatchV3BasedropCord=Instance.new('Part')	
        SourPatchV3BasedropCord.Name='SourPatchV3BasedropCord';	
        SourPatchV3BasedropCord.Anchored=true;		
        SourPatchV3BasedropCord.Parent=game.Workspace;
        SourPatchV3BasedropCord.Shape=Enum.PartType.Ball;
        SourPatchV3BasedropCord.Size=Vector3.new(2,2,2)
        SourPatchV3BasedropCord.Color=Color3.fromRGB(0,0,255);
        SourPatchV3BasedropCord.Material=Enum.Material.ForceField;
        SourPatchV3BasedropCord.CFrame=requirements.LP.Character.HumanoidRootPart.CFrame;
        SourPatchV3BasedropCord.CanCollide=false;
    end,
    true
);
...
MainPlotOptions:Toggle(
    ...
    function(value)
        if value then 
            requirements.Booleans.DisableSlotSaving=true;
            functions.notify('SourPatch','Disable if money duplicating. Enable for plot duplication',7)
            return;
        end;
        if not value then 
            requirements.Booleans.DisableSlotSaving=false;
...
MainPlotOptions:TextBox(
    ...
    function(Name)
        ...
        functions.notify('SourPatch','Saved '..tostring(Name)..'!',4);
    end,
    'Enter FileName Here'
);
...
MainPlotOptions:TextBox(
    ...
    function(Name)
        ...
        functions.notify('SourPatch','Completed '..tostring(Name)..'!',4);
    end,
    'Enter FileName Here'
);
...
MainPlotOptions:DropDown(
    ...
    {options={'SourPatch','Castle','AmongUs','Flatbed','Pyramid','TruckLog','WoodRUs','XmasTree','XWing'}}
)

MainPlotOptions:Button(
    'Build Selected Build',
    function(build)
        -- local build=game:HttpGet('https://SourPatchdevelopment.wtf/autobuilds/'..tostring(requirements.Strings.buildToBuild)..'Build');
        -- functions.loadFile(build);
        functions.notify('SourPatch','Auto-Build Temporarily Discontinued',4)
    end,
    true
)
...
MainPlotOptions:Button(
    ...
    function()
        ...
        wait(9);
        functions.notify('Page 4/4','This is REALLY BUGGY and in beta due to it being the first autobuild i\'ve ever made, report any bugs to FindFirstSourPatchツ#4476',8);
    end,
    true
);
...
MainTreeOptions:Toggle(
    ...
    function(v)
        ...
        if v then
            oldPos=requirements.LP.Character.HumanoidRootPart.CFrame;
            functions.notify('SourPatch','Loop Bring Tree Enabled',4);
        ...
        elseif not v then 
            ...
            functions.notify('SourPatch','Loop Bring Tree Disabled',4);
        end;
    end,
    false
);
...
MainTreeOptions:Toggle(
    ...
    function(v)
        ...
        functions.notify('SourPatch','Select A Sawmill',4)
        ...
        local target=requirements.Mouse.Target;
            if target then 
                ...
                if sawmill.Name:find('Sawmill')then
                    ...
                    functions.notify('SourPatch','Sawmill Selected',4)
        ...
        elseif not v then 
            ...
            functions.notify('SourPatch','Auto Mod Tree Disabled',4);
        end;
    end,
    false
);
...
MainTreeOptions:Button(
    ...
    function()
        ...
        functions.notify('SourPatch','Bring Tree took '..result..'s '..('To Bring '..requirements.Strings.bringSelectedTreeAmount)..' Tree/s',4)
        requirements.Booleans.isLoopBringTreeEnabled=false;
    end,
    true
);
...
MainTreeOptions:Button(
    ...
    function(v)
        ...
        functions.notify('SourPatch','Select A Tree And A Sawmill',4)
        connection=requirements.Mouse.Button1Down:Connect(function(b)
            local target=requirements.Mouse.Target;
            if target then 
                log=target.Parent
                if log:FindFirstChild('Owner')and log:FindFirstChild('WoodSection')and log.Parent.Name=='LogModels'then 
                    if tostring(log.Owner.Value)==tostring(requirements.LP) or log.Owner.Value==nil then
                        logModel=log
                        functions.notify('SourPatch','Tree Selected',4)
                    end;
                end;
            end;
            if target then 
        ...
        functions.notify('SourPatch','Modding Tree',4)
        if connection then
            connection:Disconnect();
            connection=nil;
        end;
                ...
                for i=1,requirements.Strings.autoBuySelectedItemAmount do
                    ...
                    axeConnection=workspace.PlayerModels.ChildAdded:Connect(function(Child)
                        ...
                        if Main:FindFirstChild'Mesh'and Main.Mesh.TextureId=='rbxassetid://273892918'then 
                            ...
                            functions.notify('SourPatch','Trying');
...
settings:KeyBind(
    ...
    function(v)
        ...
        game.CoreGui.SourPatch.Enabled=not game.CoreGui.SourPatch.Enabled;
    end,
...
settings:Button(
    ...
    function()
        if readfile('SourPatchV3.cfg') and delfile then 
            delfile('SourPatchV3.cfg')
...
settings:TextLabel('Made By SourPatch');
...
if functions.isPlayerAdmin(requirements.LP)then 
    ...
    adminFunctions.getBarkUsers=function()
        ...
        functions.notify('SourPatch','Grabbing Bark Users',4)
    ...
    adminFunctions.crashBarkUser=function(player)
        ...
        functions.notify('SourPatch','Crashing '..tostring(player),4)
        wait(15);
    ...
    local AdminOptions=AdminOptionsFrame:Section('SourPatch Admin Options');
    ...
    if requirements.Booleans.DeveloperMode then
        local AdminOptions=AdminOptionsFrame:Section('SourPatch Developer Tools');
        AdminOptions:DropDown(
            'Identify Type',
            function(plr)
        ...
        AdminOptions:Button(
            'Identify Item', 
            function()
            functions.notify('SourPatch','Select A Item',4)
            local Item=nil;
            connection=requirements.Mouse.Button1Down:Connect(function(b)
            local target=requirements.Mouse.Target;
                if target then 
                    ...
                    if requirements.Strings.IdentifyType=='Name'then 
                        functions.notify('SourPatch',tostring(target))
                        return;
                    end;
                    if requirements.Strings.IdentifyType=='CFrameValue'then 
                        if target:FindFirstChild'OriginCFrame'then
                            functions.notify('SourPatch','SetClipboard '..'{'..tostring(target.OriginCFrame.Value)..'}',4);
                    ...
                    if Item:FindFirstChild(tostring(requirements.Strings.IdentifyType))then 
                        if tostring(requirements.Strings.IdentifyType)=='Owner'and tostring(Item:FindFirstChild(tostring(requirements.Strings.IdentifyType)).Value)=='nil' then 
                            functions.notify('SourPatch','Nobody Owns This Item')
                            return;
                        end;
                        functions.notify('SourPatch',tostring(Item:FindFirstChild(tostring(requirements.Strings.IdentifyType)).Value))
                    end;
                end;
            end);
        ...
        AdminOptions:Button(
            ...
            functions.notify('SourPatch','Claimed '..tostring(Item),4)
            if connection then 
                connection:Disconnect();
                connection=nil;
            end;
        ...
        local AdminOptions=AdminOptionsFrame:Section('SourPatch Debug Tools');
        AdminOptions:Button(
            'Ping: ', 
            function()
...
game.CoreGui.ChildRemoved:Connect(function(b)
    if tostring(b)=='SourPatch'then 
        pcall(function()
            ...
            requirements.LP.Character.Head.SourPatchV3:remove();
        end);
        pcall(function()
            workspace.SourPatchV3BasedropCord:Destroy();
        end)
